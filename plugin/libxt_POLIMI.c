 ...
 */
static void polimi_print(const void *ip,
                         const struct xt_entry_target *target,
                         int numeric) {
  const struct xt_polimi_info *myinfo = (const struct xt_polimi_info *)target->data;
  printf("\n POLIMI TARGET: Replace '%s' with '%s'",
         myinfo->findString,
         myinfo->replString);
}
...
static int polimi_parse(int c, char **argv, int invert, unsigned int *flags,
  ...
  struct xt_polimi_info *myinfo = (struct xt_polimi_info *)(*target)->data;

  switch (c) {
    case 'f': { /* --findString <string>*/
      if(strlen(optarg) > POLIMI_TARGET_MAX_STRING_SIZE) {
        xtables_error(PARAMETER_PROBLEM, "findstring too long");
      }
    ...
    case 'r': { /* --replaceString <string> */
      if(strlen(optarg) > POLIMI_TARGET_MAX_STRING_SIZE) {
        xtables_error(PARAMETER_PROBLEM, "replacestring too long");
      }
